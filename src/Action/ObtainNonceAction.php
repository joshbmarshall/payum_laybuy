...
class ObtainNonceAction implements ActionInterface, GatewayAwareInterface {
    ...
    public function execute($request) {
        ...
        try {
            $customer = $model['laybuyGateway']->customer()->find($customer_id);
        } catch (\Exception $e) {
            $result = $model['laybuyGateway']->customer()->create([
                'id' => $customer_id,
                'email' => $email,
            ]);
        }
        //dump($laybuyGateway->clientToken()->generate());exit;
        $clientToken = $model['laybuyGateway']->clientToken()->generate([
